...
//    private final ActorRef<BasicCommon> kafkaConnectInActorRef;
    ...
    private List<List<Integer>> resourceDis = new ArrayList<>();
...
//        init();
        ...
        setupResourceDis(taskAndResources.get(3));
//        startBiding();
    }

    private void setupResourceDis(List<String> disStr) {
        for (int i=0; i<disStr.size(); i++) {
            List<Integer> dis =new ArrayList<>();
            String [] strs = disStr.get(i).split(",");
            for (int j=0; j<strs.length; j++) {
                dis.add(Integer.parseInt(strs[j]));
            }
            resourceDis.add(dis);
        }
    }
...
//                for (int k=0; k<GlobalAkkaPara.resourceSize; k++) {
//                    int resource = Integer.parseInt(processTimes.get(round-1).getResourceName().substring(8));
//                    StartBiding startBiding = new StartBiding();
//                    startBiding.setRound(round);
//
//                    startBiding.setStartTime(processTimes.get(round-1).getEndTime()+resourceDis.get(resource).get(k));
...
//                System.out.println("22222222222");
                StartBiding startBiding = new StartBiding();
                startBiding.setRound(round);
                startBiding.setPreResource(processTimes.get(round-1).getResourceName());
                startBiding.setStartTime(processTimes.get(round-1).getEndTime());
                startBiding.setResourceDis(resourceDis);
//                System.out.println("888888888 "+ round + " " + processTimes.get(round).getEndTime() + " " + msg.getTaskName());
                secondBrainRefMaps.get(msg.getTaskName()).tell(startBiding);
            } else {
                StartBiding startBiding = new StartBiding();
            ...
            try {
                File writeName = new File(GlobalAkkaPara.dataPath +"output"+ GlobalAkkaPara.resourceSize + "-" + (GlobalAkkaPara.taskSize*GlobalAkkaPara.taskNum) +"-" + GlobalAkkaPara.ratio + "-" +GlobalAkkaPara. rapid +".txt"); // 相对路径，如果没有则要建立一个新的output.txt文件
                if (!writeName.exists()) {
                ...
                if (round == taskProcessNum.get(msg.getTaskName())) {
//                    String s = "spendTime " + GlobalAkkaPara.systemStartTime + " " + GlobalAkkaPara.systemEndTime + " " + (GlobalAkkaPara.systemEndTime-GlobalAkkaPara.systemStartTime);
//                    out.write(s);
//                    out.newLine();
//                    out.flush(); // 把缓存区内容压入文件                System.out.println(s);
