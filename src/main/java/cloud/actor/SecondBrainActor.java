...
//        bidingMsg.setSender(refName);
        for (Map.Entry<String, ActorRef<BasicCommon>> entry : mdResourceRefMaps.entrySet()) {
            ...
            bidingMsg.setNo(bind.getRound());
            if (bind.getRound() != 0) {
                int preResourceNum = Integer.parseInt(bind.getPreResource().substring(8));
                int curResourceNum = Integer.parseInt(entry.getKey().substring(8));
                bidingMsg.setStartTime(bind.getStartTime() + bind.getResourceDis().get(preResourceNum).get(curResourceNum));
            } else {
                bidingMsg.setStartTime(bind.getStartTime());
            }
            bidingMsg.setSender(refName);
