 ...
 * @description：仿真时间窗口
 */
public class SimulateTimeWindowProcessAndFreeTimeSmallEnd {
...
//                        System.out.println( j + " " + processNo +" uuuuuuuuuuu");
                    ...
                    totalProcessCost += resourceAgents.get(selectedAgentNo).getProcessCostMap().get(processNo);

                    if (preHandleAgent != -1) {
                        totalTransferCost += transferCost[preHandleAgent][selectedAgentNo];
                    }
                    preHandleAgent = selectedAgentNo;
